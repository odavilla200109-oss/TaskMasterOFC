    ...
    <div ref={ref} style={{position:"relative"}}>
      <button onClick={()=>setOpen(o=>!o)} className="tm-btn" style={{display:"flex",alignItems:"center",gap:6,background:"rgba(16,185,129,.07)",border:"1px solid var(--border)",borderRadius:10,padding:"4px 9px 4px 5px",cursor:"pointer"}}>
        {user.photo
          ...
          :<div style={{width:26,height:26,borderRadius:"50%",background:"linear-gradient(135deg,#10b981,#059669)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:700,fontSize:11,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{user.name?.[0]?.toUpperCase()||"U"}</div>
        }
