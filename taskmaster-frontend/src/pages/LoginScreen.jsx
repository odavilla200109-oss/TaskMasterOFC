...
export function LoginScreen() {
  ...
  return (
    <div ref={bgRef} onMouseMove={onBgMove} style={{width:"100%",minHeight:"100vh",display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
      ...
      <button onClick={()=>setDark(d=>!d)} style={{position:"fixed",top:18,right:18,zIndex:200,background:"var(--bg-glass)",backdropFilter:"blur(10px)",border:"1px solid var(--border)",borderRadius:10,padding:"7px 9px",display:"flex",alignItems:"center",cursor:"pointer",color:"var(--text-sub)"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"60px 20px 40px",position:"relative",zIndex:10,gap:44}}>
        ...
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:900,fontSize:"clamp(52px,9vw,100px)",color:"var(--text-main)",letterSpacing:-3,lineHeight:1,marginBottom:18,animation:"tmFadeUp .65s ease both"}}>
            TaskMaster
          </div>
        ...
        <div style={{background:"var(--bg-card)",backdropFilter:"blur(28px) saturate(160%)",border:"1.5px solid var(--border)",borderRadius:24,padding:"32px 38px",width:"100%",maxWidth:400,boxShadow:dark?"0 12px 60px rgba(0,0,0,.6),0 0 0 1px rgba(16,185,129,.1)":"0 10px 52px rgba(16,185,129,.12),0 2px 8px rgba(0,0,0,.04)",display:"flex",flexDirection:"column",gap:13,animation:"tmScaleIn .5s cubic-bezier(.34,1.1,.64,1) .18s both"}}>
          <div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:800,fontSize:20,color:"var(--text-main)",textAlign:"center"}}>
          ...
          {loginLoading?(
            ...
            <div
              style={{
                ...
                overflow:"hidden",
                padding:dark?4:0,
                border:dark?"1px solid rgba(16,185,129,.24)":"none",
                background:dark?"rgba(6,14,10,.9)":"transparent",
              }}
            >
              <GoogleLogin
                onSuccess={handleGoogle}
                onError={()=>setError("Login Google falhou.")}
      ...
      <footer style={{position:"relative",zIndex:10,borderTop:"1px solid var(--border)",background:"var(--bg-glass)",backdropFilter:"blur(14px)",padding:"14px 36px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10,fontFamily:"'Inter',sans-serif"}}>
        <div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:800,fontSize:15,color:"var(--text-main)",letterSpacing:-0.4}}>TaskMaster</div>
